...
int Ataru(){
    ...
    while(cases--){
        ...
        ll start = 1;
        for(int i = 0; i < 63; i++){
            avail.pb(to_string(start));
            start *= 2;
        }
        //prtarr(avail, (int)avail.size());
        for(int i = 0; i < (int)avail.size(); i++){
            string cur = to_string(n);
            start = 0;
            for(int j = 0; j < (int)cur.size() && start < avail[i].length(); j++){
                if(cur[j] == avail[i][start]) start++;
            }
            //cout << start << " " << avail[i] << "\n";
            mn = min(mn, ((ll)cur.length() - start) + ((ll)avail[i].length() - start));
        }
        cout << mn << "\n";
    }
    return 0;
